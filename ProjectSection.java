...
public class ProjectSection extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        GoToControl.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                GoToControlActionPerformed(evt);
            }
        });

        GoToShop4.setFont(new java.awt.Font("Arial", 0, 12)); // NOI18N
        GoToShop4.setText("Back To Shop");
        GoToShop4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                GoToShop4ActionPerformed(evt);
            }
        });
    ...
    private void RemonveOneActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RemonveOneActionPerformed
        ...
        Product selectedProduct = products.getProducts().get(selectedRowInCart);
        ...
        if (selectedRowInCart != -1) {
            ...
            if (cartProducts.getProducts().get(selectedRowInCart).getQuantity() > 1) {
                ...
                if (i == JOptionPane.YES_OPTION) {
                    SelectedProducts.remove(selectedRowInCart);
...
int orderNumber;
    ...
    private void ConfirmTheOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ConfirmTheOrderActionPerformed
        ...
        if (selection == 0) {
            ...
            ArrayList<Product> StoreTheSelectedProducts;

            //link the order number to the order
            for (int i = 0; i <= hm.size(); i++) {
                if (hm.get(i) == null) {
                    hm.put(i + "", StoreTheSelectedProducts = (ArrayList) SelectedProducts.clone());
                    orderNumber = i;
                    StoreTheSelectedProducts = new ArrayList<>();
                    for (Map.Entry<String, ArrayList<Product>> entry : hm.entrySet()) {
            ...
            //StoreTheSelectedProducts.clear();
            ...
            SelectedProducts.removeAll(SelectedProducts);
                cartProducts.RemoveAllProducts();
    ...
    private void ShowOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ShowOrderActionPerformed
        ...
        int rowCount = OrdersTabelModel.getRowCount();
        for (int i = rowCount - 1; i >= 0; i--) {
            OrdersTabelModel.removeRow(i);
        }

        //dict.get(OrdersList.getSelectedValue());
        
        OrderTabel2.setModel(OrdersTabelModel);
        
        OrdersTabelModel.setColumnIdentifiers(new String[]{"Name", "quantity"});

        for (int i = 0; i < hm.get(OrdersList.getSelectedValue()).size(); i++) {
            recievedOrders.AddProduct(hm.get(OrdersList.getSelectedValue()).get(i).getName(),
                    hm.get(OrdersList.getSelectedValue()).get(i).getQuantity());
        }
        
        for (int i = 0; i < hm.get(OrdersList.getSelectedValue()).size(); i++) {
            OrdersTabelModel.addRow(new String[]{
                recievedOrders.getProducts().get(i).getName(),
                recievedOrders.getProducts().get(i).getQuantity() + ""
            });
        }
    ...
    //go to control page
    private void GoToControlActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GoToControlActionPerformed
        jTabbedPane1.setSelectedIndex(3);
    }//GEN-LAST:event_GoToControlActionPerformed

    //go to shop
    private void GoToShop4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GoToShop4ActionPerformed
        jTabbedPane1.setSelectedIndex(0);
    }//GEN-LAST:event_GoToShop4ActionPerformed
